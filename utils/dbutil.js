...
Table.prototype.update = function(values, callback) {
    ...
    if (this.clearTable(values)) {
        this.getConnection(function(connection) {
            var query = connection.query("update  " + me.tablename + " set ? where id=" + connection.escape(values.id || values.ID), values, function(err, result) {
                if (err) {
                    callback(err,result);
                }else{
